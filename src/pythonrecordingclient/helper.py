...
class BugException(Exception):  # noqa: N818
    def __init__(
        self,
        msg="You have found a bug. Please contact a developer with the command and all output.",
        *args,
        **kwargs,
    ):
...
def obj_to_xml(oName, obj):
    ...
    for name, value in lists.items():
        for item in value:
            res += obj_to_xml(name, item)
    for name, value in elements.items():
        res += obj_to_xml(name, value)
    res += "</" + oName + ">"
    return res


def to_stamp(dt: datetime) -> int:
