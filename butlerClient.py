...
def set_graph(args):
    ...
    res = requests.post(
        args.url + "/" + args.api + "/start_dialog", json=json.dumps(d), cookies={"_forward_auth": args.token}
    )
    if res.status_code != 200:
...
def run_session(args, audio_source):
    ...
    t = Thread(target=read_text, args=(args.url, sessionID, args.api, args.token))
    t.daemon = True
    t.start()
    ...
    info = requests.post(
        args.url + "/" + args.api + "/" + sessionID + "/" + voice_stream_ID + "/append",
        json=json.dumps(data),
